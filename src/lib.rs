...
impl OneshotRingBuffer {
    ...
    pub unsafe fn prepare_send(&self, len: usize) -> &mut [u8] {
        assert!(len != 0);
        ...
        let mut start = None;
        for i in 0..len {
            let ptr = unsafe { UnsafeCell::raw_get(self.buffer.as_ptr().add(idx + i)) };
            if start.is_none() {
                start = Some(ptr);
            }
        }
        ...
        unsafe { std::slice::from_raw_parts_mut(start.unwrap(), len) }
    }
    ...
    pub unsafe fn send(&self, count: usize) {
    ...
    pub unsafe fn recv(&self) -> Option<&[u8]> {
        loop {
            ...
            if head != tail {
                ...
                let mut start = None;
                for i in 0..len {
                    let ptr = unsafe { UnsafeCell::raw_get(self.buffer.as_ptr().add(tail + i)) };
                    if start.is_none() {
                        start = Some(ptr);
                    }
                }
                ...
                break Some(unsafe { std::slice::from_raw_parts(start.unwrap(), len) });
            }
            std::hint::spin_loop();
        }
    }
